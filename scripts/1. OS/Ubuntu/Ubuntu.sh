...
check01() {
    ...
    if [ ! -f "$SSHD_CONFIG" ]; then
        ...
        if [[ "${PERMIT,,}" == "no" ]]; then
            ...
            RES="N"
            DESC="PermitRootLogin 설정이 없음 (기본값 허용)"
...
check03() {
    ...
    # PAM 설정 확인 (Ubuntu: common-auth)
    if [ -z "$DENY_VALUE" ]; then
        DENY_VALUE=$(grep -E "pam_faillock.*deny=" /etc/pam.d/common-auth 2>/dev/null | grep -oP 'deny=\K[0-9]+' | head -1)
        if [ -n "$DENY_VALUE" ]; then
            DETAILS="pam_faillock deny: $DENY_VALUE (common-auth)"
        fi
    fi

    # pam_tally2 확인 (Ubuntu 구버전)
    if [ -z "$DENY_VALUE" ]; then
        DENY_VALUE=$(grep -E "pam_tally2.*deny=" /etc/pam.d/common-auth 2>/dev/null | grep -oP 'deny=\K[0-9]+' | head -1)
        if [ -n "$DENY_VALUE" ]; then
            DETAILS="pam_tally2 deny: $DENY_VALUE (common-auth)"
        fi
    fi
...
check17() {
    ...
    for target in $TARGETS; do
        if [ -e "$target" ]; then
            ...
            CHECKED="${CHECKED}${target}(${owner}:${perm}) "
    ...
    if [ -z "$VULNERABLE" ]; then
        ...
        DT="검사 대상: $CHECKED| 취약 파일: 없음"
    else
        RES="N"
        DESC="시스템 시작 스크립트 권한 부적절"
        DT="검사 대상: $CHECKED| 취약 파일: $VULNERABLE"
    fi
...
check25() {
    ...
    # 검사 대상 디렉토리
    local SEARCH_DIRS="/etc /var"
    # world writable 파일 검색 (주요 디렉토리)
    local WW_FILES=$(find /etc /var -type f -perm -2 2>/dev/null | head -10)
    local WW_COUNT=$(find /etc /var -type f -perm -2 2>/dev/null | wc -l)

    if [ -z "$WW_FILES" ]; then
        RES="Y"
        DESC="world writable 파일이 존재하지 않음"
        DT="검사 대상: $SEARCH_DIRS | world writable 파일: 없음"
    else
        RES="N"
        DESC="world writable 파일이 존재함"
        DT="검사 대상: $SEARCH_DIRS | world writable 파일(총 ${WW_COUNT}개, 상위 10개):\n$WW_FILES"
    fi
...
check26() {
    ...
    if [ -z "$DEV_FILES" ]; then
        ...
        DT="검사 범위: / (제외: $EXCLUDE_PATHS) | 비정상 device 파일: 없음"
    else
        RES="N"
        DESC="/dev 외부에 device 파일 존재"
        DT="검사 범위: / (제외: $EXCLUDE_PATHS) | 비정상 device 파일(총 ${DEV_COUNT}개, 상위 10개):\n$DEV_FILES"
    fi
...
check27() {
    ...
    if [ -z "$VULNERABLE" ]; then
        RES="Y"
        DESC=".rhosts, hosts.equiv 파일이 없음"
        DT="취약 파일: 없음"
    else
        RES="N"
        DESC=".rhosts, hosts.equiv 파일 존재"
        DT="$DETAILS"
    fi
...
check28() {
    ...
    if command -v ufw &>/dev/null; then
        ...
        DETAILS="${DETAILS}ufw: $UFW_STATUS\n"
...
check38() {
    ...
    for svc in $VULNERABLE_SVCS; do
        if grep -rq "^$svc" /etc/xinetd.d/ 2>/dev/null; then
            local disabled=$(grep -l "^$svc" /etc/xinetd.d/* | xargs grep "disable.*=.*yes" 2>/dev/null)
            if [ -z "$disabled" ]; then
                RUNNING="${RUNNING}${svc} "
            fi
        fi
...
check43() {
    ...
    # systemd 서비스 확인 (RHEL: ypbind, Ubuntu: nis)
    for svc_unit in ypbind nis; do
        if systemctl is-active "$svc_unit" &>/dev/null; then
            RUNNING=true
            DETAILS="${DETAILS}${svc_unit}.service: 실행 중\n"
        fi
    done
...
check44() {
    ...
    # tftp 확인
    if pgrep -x "in.tftpd\|tftpd" &>/dev/null || ss -tuln | grep -q ":69 "; then
        RUNNING="${RUNNING}tftp "
    fi

    # talk 확인
    if pgrep -x "in.talkd\|talkd" &>/dev/null || ss -tuln | grep -q ":517 \|:518 "; then
...
check46() {
    ...
    if [ -z "$SENDMAIL_PATH" ]; then
        ...
        if $HAS_ISSUE; then
            RES="N"
            DESC="sendmail 보안 설정 미흡"
        else
            RES="Y"
            DESC="sendmail 보안 설정 양호"
        fi
        DT="$DETAILS"
    fi
...
check48() {
    ...
    # sendmail 확인
    if [ -f /etc/mail/sendmail.cf ]; then
        local PRIVACY=$(grep -i "PrivacyOptions" /etc/mail/sendmail.cf)
        DETAILS="sendmail PrivacyOptions:\n$PRIVACY"

        if echo "$PRIVACY" | grep -qi "noexpn\|novrfy"; then
            RES="Y"
...
check50() {
    ...
    # DNS 설정 파일 (RHEL: /etc/named.conf, Ubuntu: /etc/bind/named.conf)
    local NAMED_CONF=""
    ...
    if [ -z "$NAMED_CONF" ]; then
        ...
        local ALLOW_TRANSFER=$(grep -i "allow-transfer" "$NAMED_CONF")

        if [ -z "$ALLOW_TRANSFER" ]; then
            RES="N"
            DESC="Zone Transfer 제한 미설정"
            DT="allow-transfer: not set"
        elif echo "$ALLOW_TRANSFER" | grep -q "none\|specific"; then
            RES="Y"
            DESC="Zone Transfer가 제한됨"
            DT="$ALLOW_TRANSFER"
        else
            RES="M"
...
check62() {
    ...
    if $HAS_BANNER; then
        RES="M"
        DESC="경고 메시지 설정 확인됨 - 내용 적절성 수동 확인 필요"
        DETAILS="${DETAILS}\n※ 경고 메시지에 OS 버전 등 서버 정보가 포함되어 있는지 확인하세요."
    else
        RES="N"
...
check66() {
    ...
    if [ -n "$AUTHLOG" ]; then
        ...
        RES="M"
        DESC="rsyslog 실행 중, 설정 수동 확인 필요"
        DETAILS="${DETAILS}\nrsyslog: 실행 중"
    else
        RES="N"
        DESC="rsyslog 서비스가 실행되지 않음"
        DETAILS="rsyslog: 미실행"
    fi
...
check67() {
    ...
    for log in $LOG_FILES; do
        ...
        if [ -f "$LOG_PATH" ]; then
            ...
            # 가이드라인 기준: 소유자가 root가 아니거나 권한이 644 초과 시 취약
            if [ "$owner" != "root" ] || [ "$perm" -gt 644 ] 2>/dev/null; then
